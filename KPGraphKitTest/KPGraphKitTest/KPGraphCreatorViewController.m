//
//  KPGraphCreatorViewController.m
//  KPGraphKitTest
//
...
@interface KPGraphCreatorViewController ()

@end

@implementation KPGraphCreatorViewController
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (alertView == firstNodeAlert) {
        ...
        [self dijkstrasWithStart:startID end:endID];
    }
}

-(IBAction)pushRunAlgorithm:(id)sender {
    switch (graphType) {
        case KPGraphTypeDijkstras:
    ...
    [resolver dijkstrasOnConnections:connections
                ...
                andCompletionHandler:^(NSMutableArray *path, NSInteger weight) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        ...
                        [[self navigationItem] setTitle:[NSString stringWithFormat:@"Route Found With Weight %i", weight]];
...
- (void)viewDidLoad
{
    if (graphType == KPGraphTypeDijkstras) {
        [self.navigationItem setTitle:@"Dijkstra's"];
    }
    
    resolver = [[KPGraphResolver alloc] init];
